        ...
        if isPressed then
            ...
            if currTime >= whenToDepress then
                ...
                if lastNeedDepress then
                    local ok_dep, err_dep = pcall(function()
                        GetDevice(lastDevice):performClickableAction(lastCode, 0)
                    end)
                    if not ok_dep then
                        log.write("THEWAY", log.ERROR, "depress failed: " .. tostring(err_dep))
                    end
                end
                isPressed = false
            ...
            if currCommandIndex <= #keys then
                ...
                if not ok_click then
                    log.write("THEWAY", log.ERROR, "performClickableAction failed: " .. tostring(err_click))
                end
